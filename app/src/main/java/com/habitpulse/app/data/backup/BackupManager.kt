package com.habitpulse.app.data.backup
...
class BackupManager(
    ...
    suspend fun exportTo(uri: Uri): Boolean {
        ...
        val text = json.encodeToString(HabitPulseBackup.serializer(), backup)
        return runCatching {
    ...
    suspend fun importFrom(uri: Uri, mode: ImportMode): ImportResult {
        ...
        val backup = try {
            json.decodeFromString(HabitPulseBackup.serializer(), text)
